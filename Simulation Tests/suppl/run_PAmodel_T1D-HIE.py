...
# This script runs the exercise model for the high-intensity exercise study by Jayawardene (2017) used for model
# calibration and generates the corresponding figure (Fig. S4) of the following manuscript:
#
...
params = pd.read_csv('parameters/params_T1D-V3.csv', header=None,
                     dtype={0: str}, delimiter=';').set_index(0).squeeze().to_dict()


''' Import data '''

data = pd.read_csv('data/data_Jayawardene.csv')
input = pd.read_csv('data/input_Jayawardene.csv')
info = pd.read_csv('data/info_Jayawardene.csv')
...
plot.plot_glc(t, model['G'], Gb, pa, glc_data=data[['time', 'Glc']], glc_sd=data['Glc_sd'])
plt.show()
